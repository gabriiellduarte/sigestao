...
export default function Edit({ grupo }: EditProps) {
  const { data, setData, put, processing, errors } = useForm({
    reg_gpro_nome: grupo.reg_gpro_nome
  });

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    put(route('regulacao.grupoprocedimentos.update', grupo.reg_gpro_id));
  };
    ...
    <AppLayout>
      <div className="space-y-6 p-4 max-w-xl mx-auto">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold text-blue-800">Editar Grupo de Procedimento</h1>
        </div>
        <Card>
          <CardHeader>
